...
def graficar_dominio_temporal(signal,fs, hilbert=False, ):
    ...
    plt.tight_layout()
    if hilbert:
        out_file = get_output_filepath(f'time_domain_plot_hilbert.png',2,('static','img','temp'))
    else:
        out_file = get_output_filepath(f'time_domain_plot.png',2,('static','img','temp'))
...
def graficar_espectro(signal, fs,color='r'):
    ...
    plt.tight_layout()
    out_file = get_output_filepath(f'freq_domain_plot.png',2,('static','img','temp'))
    plt.savefig(out_file,dpi=300,bbox_inches="tight")
...
def graficar_resultados(freq, datos_debug,fs):    
    ...
    plt.axhline(nivel_ruido, color='r',linewidth=3, linestyle=':', label=f'Nivel de Ruido ({nivel_ruido:.1f} dB)')
    plt.axvline(tiempo_cruce, color='g',linewidth=3, linestyle=':', label=f'Cruce Lundeby ({tiempo_cruce:.2f} s)')
