...
def ruidoRosa_voss_editado(t, fs=44100, ncols=16):
    ...
    out_file = get_output_filepath(
        'ruido_rosa.wav', 3, ('audios', 'temp')
    )
    sf.write(out_file, total, fs)
    
    return total

def generar_sweep_inverse(duracion,fs=44100 ,f_inferior=20 ,f_superior=20000, write_wav=False):
    ...
    write_wav: bool
        Permite guardar archivo wav con los datos solicitados, por defecto es False
    ...
    if write_wav:
        out_file_sweep = get_output_filepath('sweep.wav', 3, ('audios', 'temp'))
        out_file_inverse = get_output_filepath('inverse_filter.wav',3,('audios','temp'))

        #write wav
        sf.write(out_file_sweep,sweep,fs)
        sf.write(out_file_inverse,inverse_sweep,fs)
...
def grabar_reproducir(signal,fs=44100):
    ...
    filename = get_output_filepath('play_rec.wav', 3, ('audios', 'temp'))
    sf.write(filename, grabacion, fs) #guardo la grabacion
